          ...
          <TableBody>
            ...
            {slice.map((lead) => (
              <TableRow key={lead.id} className="hover:bg-gray-50">
                ...
                <TableCell>
                  {lead.loan_type ? (
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${LOAN_COLORS[lead.loan_type] ?? "bg-gray-100 text-gray-600"}`}>
                      {lead.loan_type.charAt(0).toUpperCase() + lead.loan_type.slice(1)}
                    </span>
                  ) : "—"}
                </TableCell>
                <TableCell className="text-sm text-gray-700">{formatINR(lead.loan_amount)}</TableCell>
