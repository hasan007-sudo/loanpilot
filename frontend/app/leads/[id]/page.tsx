...
export default async function LeadDetailPage({ params }: PageProps) {
  ...
  const fields = [
    ...
    { label: "Loan Type", value: lead.loan_type ? lead.loan_type.charAt(0).toUpperCase() + lead.loan_type.slice(1) : "—" },
    { label: "Loan Amount", value: formatINR(lead.loan_amount) },
    { label: "Monthly Income", value: lead.monthly_income ? `₹${lead.monthly_income.toLocaleString("en-IN")}` : "—" },
    { label: "Employment", value: lead.employment_type?.replace("_", " ") ?? "—" },
    { label: "Bolna Call ID", value: lead.bolna_call_id ?? "—" },
